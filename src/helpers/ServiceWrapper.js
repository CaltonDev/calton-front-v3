...
import {setAverageVotoByTime, setAverageVotoIsLoading} from "../redux/home/AverageVotoByTime";
import {setAverageSentimentByTime, setAverageSentimentIsLoading} from "../redux/home/AverageSentimentByTime";
import {setAverageReviewByTime, setAverageReviewIsLoading} from "../redux/home/AverageReviewByTime";
import {showToast} from "../redux/errorToast";
import {setSources, setSourcesIsLoading} from "../redux/home/Sources";
...
import {
    ...
    setAllSources,
    setAllTopics
} from "../redux/SelectableFilters";
import {setBubbleIsLoading, setBubbles} from "../redux/home/Bubble";
import {setFeedbackIsLoading, setFeedbacks, setFeedbacksCount, setLoadingCount} from "../redux/home/FeedbackHome";
import LocationService from "../services/LocationService";
import {setLocationFiltered} from "../redux/LocationFiltered";
import LoginService from "../services/LoginService";
import {setChildUsers} from "../redux/ChildUsers";
import SourcesService from "../services/SourcesService";
import {setSourcesFiltered} from "../redux/SourcesFiltered";
import FeedbackService from "../services/FeedbackService";
import {getCodeOnlyCompetitors, getNoCodeFromPlatfrom} from "./helpers";
import {setDistribuzioniRacc, setDistribuzioniRaccIsLoading} from "../redux/home/DistribuzioneRaccomandazioni";
import AnalisiAvanzataService from "../services/AnalisiAvanzataService";
import {
    setAvgRating, setAvgSentiment, setBestWorst,
    setCountTopic,
    setDistribuzioneSentiment,
    setDistribuzioneTopic,
    setDistribuzioneTopicPerSentiment, setReviewsInfos,
    setSentimentChips,
    setTopics,
...
import { setMenusList } from '../redux/Menus';
import { setDistribuzioniVoti, setDistribuzioniVotiIsLoading } from '../redux/home/DistribuzioniVoti';
...
async function wrapperLoadTopics(allFilters,
                                 dispatch,
                                 code
) {
    ...
    await AnalisiAvanzataService.getTopicFiltered(
        true,
        dispatch
    );
    AnalisiAvanzataService.getDistribuzioneTopic(allFilters, code, dispatch, true);
    ...
    await Promise.all([
        AnalisiAvanzataService.getDistribuzioneSentiment(allFilters, code, dispatch, true),
        ...
        AnalisiAvanzataService.getDistribuzioneTopicPerSentiment(allFilters, code, dispatch, true),
        AnalisiAvanzataService.getDistribuzioneTopicPerData(allFilters, code, dispatch, true),
        AnalisiAvanzataService.getVotoMedioTopic(allFilters, code, dispatch, true),
        AnalisiAvanzataService.getVotoMedioTopicPerData(allFilters, code, dispatch, true),
        TopicService.sentTopAndWorst(allFilters, code, dispatch)
    ])
}

async function wrapperReloadHome(allFilters, columnDateToGroup, wordSelected, dispatch, numCommons, compactValues, t) {
    ...
    wrapperGetSourcesHome(allFilters, dispatch, getNoCodeFromPlatfrom(), true, true, t)
    wrapperGetAverageVotoByTime(allFilters, "rating", "Date", dispatch, true, t)
    wrapperGetAverageSentimentByTime(allFilters, "sentiment",undefined, dispatch, true, t)
...
async function wrapperDistribuzioneVoti(allFilters, columnDateToGroup, returnAnt, compactValues, t, dispatch) {
    try {
        const response = await HomeService.getDistribuzioneVoti(allFilters, columnDateToGroup, undefined, returnAnt, undefined, compactValues)
        if (response.data) {
            dispatch(setDistribuzioniVoti(response.data.data))
        }
    } catch (e) {
        dispatch(showToast({type: 2, text: t("Impossibile ottenere distribuzione voti")}))
...
async function wrapperDistribuzioneRecc(allFilters, columnDateToGroup, dispatch, returnAnt) {
    try {
        const response = await HomeService.getDistributionReccomandation(allFilters, ['tipo_raccomandazione'], columnDateToGroup, getNoCodeFromPlatfrom(), returnAnt)
        if (response.data) {
...
async function wrapperGetAverageVotoByTime(allFilters, type, columnDateToGroup, dispatch, returnAnt,t) {
    try {
        const response = await HomeService.getAverageByTime(allFilters, type, columnDateToGroup, getNoCodeFromPlatfrom(), undefined, returnAnt, undefined)
        if (response.data) {
            dispatch(setAverageVotoByTime(response.data.data))
        } else {
            dispatch(showToast({type: 2, text: t("Impossibile ottenere media voti")}))
        }
    } catch (e) {
        dispatch(showToast({type: 2, text: t("Impossibile ottenere media voti")}))
...
async function wrapperGetAverageSentimentByTime(allFilters, type, columnDateToGroup, dispatch, returnAnt, t) {
    try {
        const response = await HomeService.getAverageByTime(allFilters, type, columnDateToGroup, getNoCodeFromPlatfrom(), undefined, returnAnt, undefined)
        if (response.data) {
            dispatch(setAverageSentimentByTime(response.data.data))
        } else {
            dispatch(showToast({type: 2, text: t("Impossibile ottenere media sentiment")}))
        }
    } catch (e) {
        dispatch(showToast({type: 2, text: t("Impossibile ottenere media sentiment")}))
...
async function wrapperGetAverageReviewByTime(allFilters, dispatch, returnAnt,t) {
    try {
        const response = await HomeService.distribuzioneRecensioniPerData(allFilters, getNoCodeFromPlatfrom(), undefined, returnAnt)
        if (response.data) {
            dispatch(setAverageReviewByTime(response.data))
        } else {
            dispatch(showToast({type: 2, text: t("Impossibile ottenere media recensioni")}))
        }
    } catch (e) {
        dispatch(showToast({type: 2, text: t("Impossibile ottenere media recensioni")}))
...
async function wrapperGetSourcesHome(allFilters, dispatch, nocode = getNoCodeFromPlatfrom(), returnAnt, fromHome, t) {
    try {
        const response = await HomeService.getSourcesHome(allFilters, nocode, null, null, null, returnAnt, fromHome)
        if (response.data) {
...
async function wrapperGetSourcesFilter(allFilters, dispatch, nocode = getNoCodeFromPlatfrom(), t) {

    try {
        const response = await HomeService.getSourcesHome(allFilters, nocode, undefined, undefined, undefined, undefined)
...
async function wrappeWordsCountBUBBLE(allFilters, dispatch, numCommons, t) {
    try {
        const response = await HomeService.wordsCountBUBBLE(allFilters, numCommons, getNoCodeFromPlatfrom())
        if (response.data) {
            dispatch(setBubbles(response.data.data))
        } else {
            dispatch(showToast({type: 2, text: t("Impossibile ottenere bubbles")}))
        }
    } catch (e) {
        dispatch(showToast({type: 2, text: t("Impossibile ottenere bubbles")}))
        dispatch(setBubbles([]))
...
async function wrapperGetFeedbackSimplifiedHome(allFilters, wordSelected, dispatch) {
    try {
        const response = await HomeService.getFeedbackSimplifiedHome(allFilters, wordSelected, getNoCodeFromPlatfrom(), false, true)
        if (response.data) {
...
async function wrapperGetInfoFeedbackHome(allFilters, wordSelected, dispatch) {
    try {
        const response = await HomeService.getInfoFeedbackHome(allFilters, wordSelected, getNoCodeFromPlatfrom(), true)
        if (response.data) {
...
async function wrapperGetLocationsFiltered(dispatch) {
    try {
        ...
        const response = await FilterService.getLocationsFiltered(
            platformType === "surveys" ? [0, 1, 2, 3, 4] : getNoCodeFromPlatfrom()
        )
        if (response.data) {
...
async function wrappeGetProductsService(dispatch, platformType, t) {
    try {
        let response
        const code = platformType === 'competitor' ? getCodeOnlyCompetitors() : getNoCodeFromPlatfrom();
        ...
        if (response.data) {
            ...
            dispatch(showToast({type: 2, text: t("Impossibile ottenere products filtered")}))
...
async function wrappeGetLocationService(dispatch, platformType = 'reviews', t) {
    try {
        ...
        if (response.data) {
            ...
            dispatch(showToast({type: 2, text: t("Impossibile ottenere location filtered")}))
...
async function wrappeGetChildUsers(dispatch, t) {
    try {
        ...
        if (response.data) {
            ...
            dispatch(showToast({type: 2, text: t("Impossibile ottenere utenti figli")}))
        }
    } catch (e) {
        dispatch(showToast({type: 2, text: t("Impossibile ottenere utenti figli")}))
...
async function wrappeRegisterSubAccount(dispatch, newUser, t) {
    try {
        ...
        if (response.data && response.data.code == "200") {
            dispatch(showToast({type: 0, text: t("Utente aggiunto con successo")}))
            wrappeGetChildUsers(dispatch, t)
        } else {
            dispatch(showToast({type: 2, text: t("Impossibile completare la registrazione")}))
        }
        return response
    } catch (e) {
...
async function wrapperEditChildUser(dispatch, newUser,  t) {
    try {
        ...
        if (response.data && response.data.code == "200") {
            dispatch(showToast({type: 0, text: t("Utente modificato con successo")}))
            wrappeGetChildUsers(dispatch, t)
        } else {
            dispatch(showToast({type: 2, text: t("Impossibile completare la modifica")}))
        }
    } catch (e) {
        dispatch(showToast({type: 2, text: t("Impossibile completare la modifica")}))
...
async function wrapperDeleteChildUser(dispatch, body, t) {
    try {
        ...
        if (response.data && response?.data?.code === "200") {
            dispatch(showToast({type: 0, text: t("Utente eliminato con successo")}))
            wrappeGetChildUsers(dispatch, t)
        } else {
            dispatch(showToast({type: 2, text: t("Impossibile completare l'eliminazione")}))
        }
    } catch (e) {
        dispatch(showToast({type: 2, text: t("Impossibile completare l'eliminazione")}))
...
async function wrapperGetSourcesFiltered(dispatch, code, t) {
    try {
        ...
        dispatch(showToast({type: 2, text: t("Impossibile ottenere sources")}))

    }
}

async function wrapperGetFeedbacks(allFilters, dispatch, simplified, skip, limit, sort, search, download, typeDownload, code = getNoCodeFromPlatfrom(), t) {
    try {
...
async function wrapperGetInfoFeedbacks(allFilters, dispatch, simplified, skip, limit, sort, search, download, typeDownload, code = getNoCodeFromPlatfrom()) {
    try {
