export function handleAddingMoreTimeInterval(weekday, index, distinctPeriod, setDistinctPeriod) {
...
export function handleDeleteMoreHoursTimeInterval(weekday, periodIndex, hourIndex, distinctPeriod, setDistinctPeriod) {
  let tmp = JSON.parse(JSON.stringify(distinctPeriod))
...
export function handleAddingSpecialTimeInterval(startDate, endDate, specialPeriods, setSpecialPeriods, index) {
...
export function handleAddingMoreHoursTimeInterval(weekday, listing, setListing) {
  const newPeriod = {
    "openHours": null,
    "openMinutes": null,
    "closeHours": null,
...
export function handleDeleteTimeInterval(weekday, startTime, endTime, index, setListing, listing) {
  let currentList = JSON.parse(JSON.stringify(listing))
  currentList[weekday].splice(index, 1)
  setListing(currentList)
}

export function handleDeleteSpecialTimeInterval(hourIndex, periodIndex, specialPeriods, setSpecialPeriods) {
  let tmpPeriod = [...specialPeriods]
