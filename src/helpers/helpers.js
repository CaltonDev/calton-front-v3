...
import { popSocketMessage, pushSocketMessage, resetSocketMessage, setConsumeLocally } from '../redux/Socket';
...
export function getSorter(element, type) {
    ...
    if(type === 'date') {
        return (a, b) => moment(a[element.key])?.unix() - moment(b[element.key]).unix()
    }
}
...
export function displayVariationsFromObj(obj) {
    ...
    for (let key in obj) {
        const value = obj[key];
        productNewName.push(capitalizeWords(`${key.replace('_', ' ')}: ${value}`));
    }
...
export function getNoCodeFromPlatfrom() {
    ...
    return platformType === 'reviews' ? AppConfig.codesource : platformType ==='competitor' ? AppConfig.codecompetitor : platformType === 'listing' ? AppConfig.codelisting : AppConfig.codesurvey
}
...
export function getAllCode() {;
    return AppConfig.codeonlycompetitor.concat(AppConfig.codesource, AppConfig.codelisting, AppConfig.codesurvey)
}

export function getCodeFromPlatfrom() {
    const platformType = store.getState()?.Settings.platformType
    return platformType === 'reviews' ? AppConfig.codesource : AppConfig.codesurvey
}
        ...
        for (const item of items){
            for (const [key, value] of Object.entries(item)) {
                if (typeof value === 'string' || value instanceof String) {
                    ...
                    if (similarity >= 0.5 || String(value)?.toLowerCase().includes(search?.toLowerCase()?.trim())) {
                        const exist = tmpFonti?.find((elm) => elm?._id === item?._id)
                        if(!exist) {
                            tmpFonti.push(item)
                        }
                    }
...
export async function manageLocalToastWSAndReload(socketMessage, reloadData, dispatch, t, additionalCheckReload = true) {
...
export async function manageLayoutToastWSAndReload(socketMessage, reloadData, dispatch, t) {
    let currData = []
    ...
    if (remains === 0) {
        if (error === tot) {
            ...
            dispatch(showToast({type: 6, idTask: idTask, pendingText: message, position: "top-right"}));
        } else if (error < tot && error > 0) {
            // at least one operation went wrong - Warning message
            dispatch(showToast({type: 8, idTask: idTask, pendingText: message, position: "top-right"}));
        } else {
            // all operations went well - Success message
            dispatch(showToast({type: 5, idTask: idTask, pendingText: message, position: "top-right"}));
        }

        dispatch(setConsumeLocally(true))
    } else {
        popMessageFromSocket(socketMessage, dispatch)
        dispatch(showToast({type: 4, idTask: idTask, pendingText: message, position: "top-right"}))
    }
}
