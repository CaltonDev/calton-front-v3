...
function PostOffer({
    ...
    const CustomInput = forwardRef<HTMLInputElement, CustomInputProps>(
        ({ value, onClick }, ref) => (
            <input
                className={
                    isFromPopper ? styles.customInputPopper : styles.customInput
                }
                readOnly
                onClick={onClick}
                ref={ref}
    ...
    const handleSetStartDate = (date: Date) => {
        ...
        if (date !== null) {
            ...
            const timeObj = {
                endTime: {
                    hours: tmpPost?.event?.schedule?.endTime
                        ? tmpPost?.event?.schedule?.endTime.hours
                        : '00',
                    minutes: tmpPost?.event?.schedule?.endTime
                        ? tmpPost?.event?.schedule?.endTime.minutes
                        : '00',
                },
                startTime: {
                    hours: tmpPost?.event?.schedule?.startTime
                        ? tmpPost?.event?.schedule?.startTime.hours
                        : '00',
                    minutes: tmpPost?.event?.schedule?.startTime
                        ? tmpPost?.event?.schedule?.startTime.minutes
                        : '00',
                },
    ...
    const handleSetOfferDetails = (value: string) => {
        const tmpPost = JSON.parse(JSON.stringify(post))
        tmpPost.summary = value
        setPost(tmpPost)
    }

    const handleSetOfferTerms = (value: string) => {
        const tmpPost = JSON.parse(JSON.stringify(post))
        if (tmpPost?.offer) {
    ...
    const handleSetCouponCode = (value: string) => {
        const tmpPost = JSON.parse(JSON.stringify(post))
        if (tmpPost?.offer) {
    ...
    const handleSetOfferLink = (value: string) => {
        const tmpPost = JSON.parse(JSON.stringify(post))
        if (tmpPost?.offer) {
    ...
    const handleValidateFormikImgData = (data: any) => {
        ...
        >
                ...
                >
                    ...
                    >
                        <div
                            ...
                            style={isFromPopper ? { paddingRight: 20 } : {}}
                        >
                ...
                <div
                    className={
                        isFromPopper
                            ...
                            : styles.containerInputRightItem
                    }
                    style={isFromPopper ? { width: '100%' } : {}}
                >
                    ...
                    <div style={{ display: 'flex' }}>
                        <PostCarousel
                            handleFormikValidation={handleValidateFormikImgData}
            ...
            {!isFromPopper && (
                <div style={{ marginTop: 20 }}>
                    ...
                    >
                        {!showMore
                            ? t('Aggiungi altri dettagli (facoltativo)')
                            : t('Nascondi')}
                    </span>
                    {!showMore ? (
                ...
                <div style={{ display: 'flex', marginTop: 20 }}>
                    <div className={styles.containerInputAdditionalFields}>
                        <div style={{ display: 'flex' }}>
                            ...
                            <div className={styles.containerInput}>
                                <span className={styles.label}>
                                    {t("Link per utilizzare l'offerta")}
                                </span>
                                <Input
                                    type={'text'}
                                    style={{ width: '95%' }}
