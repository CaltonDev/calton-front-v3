...
function ListingEditMenu() {
  ...
  function cardsToFitInView(viewWidth, cardWidth) {
    ...
    const numberOfCards = Math.round(viewWidth / cardWidth) !== 0 ? Math.round(viewWidth / cardWidth): 1;

    return numberOfCards === 1? 2: numberOfCards > 5? 5:numberOfCards;
  }

  const [numberOfCardsOnLoader, setNumberOfCardsOnLoader] = useState(cardsToFitInView(userViewWidth, 400))
