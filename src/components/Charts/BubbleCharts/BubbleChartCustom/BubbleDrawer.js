...
export default class BubbleDrawer {
    ...
    handleTick = () => {
        ...
        this.rectNeutral
            .attr("x", (d) => {
                const percentage = (d.count.positive + d.count.negative) / 100;
                const width = d.r * 2;
        ...
        this.rectNegative
            .attr("x", (d) => {
                const percentage = (d.count.positive + d.count.neutral) / 100;
                const width = d.r * 2.2;
                const result = width * percentage;
                const start = d.x - d.r;
                return start + result;
    ...
    handleClick = (d, sentiment= 1, isText = false) => {
        ...
        onBubbleClick && onBubbleClick(d.target?.attributes?.name?.value, sentiment, isText);
    };
    ...
    focus = (skillId, type) => {
        const { svg } = this.props;
        const node =
            svg && svg.select(`#bubble${skillId}${type}`).node();
    ...
    highlightType = (type = null) => {
        ...
        const alpha = type
            ? (count.positive + count.negative + count.neutral) / count[type] / 2
            : 1;
        bubbles.each((d) => {
